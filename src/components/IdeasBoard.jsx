...
const COLOR_STYLES = {
    slate: { dot: 'bg-slate-500', text: 'text-slate-700', ring: 'ring-slate-200' },
    sky: { dot: 'bg-sky-500', text: 'text-sky-700', ring: 'ring-sky-200' },
    teal: { dot: 'bg-teal-500', text: 'text-teal-700', ring: 'ring-teal-200' },
    ...
    orange: { dot: 'bg-orange-500', text: 'text-orange-700', ring: 'ring-orange-200' },
    rose: { dot: 'bg-rose-500', text: 'text-rose-700', ring: 'ring-rose-200' },
    pink: { dot: 'bg-pink-500', text: 'text-pink-700', ring: 'ring-pink-200' },
    violet: { dot: 'bg-violet-500', text: 'text-violet-700', ring: 'ring-violet-200' },
    cyan: { dot: 'bg-cyan-500', text: 'text-cyan-700', ring: 'ring-cyan-200' }
};
        ...
        <div>
            <button
                ...
                className={`flex w-full items-start gap-2 rounded-lg px-2 py-2 text-left transition ${
                    selected ? `bg-white ring-1 ${colors.ring}` : 'hover:bg-white/70'
                }`}
                style={{ paddingLeft: `${8 + depth * 18}px` }}
            >
                <span className={`mt-2 h-2 w-2 shrink-0 rounded-full ${colors.dot}`} />
                <span className="min-w-0 flex-1">
                    <span className={`block truncate text-sm font-semibold ${node.completed ? 'text-slate-400 line-through' : 'text-slate-850'}`}>
                        {node.title}
                    </span>
                    {node.details && depth === 0 && (
                        <span className="mt-0.5 block truncate text-xs text-slate-500">{node.details}</span>
                    )}
                </span>
    ...
    return (
        <>
            <input
                ...
                className={`w-full bg-transparent text-3xl font-bold leading-tight outline-none ${node.completed ? 'text-slate-400 line-through' : 'text-slate-950'}`}
                placeholder="Untitled idea"
            />

            <div className="mt-5 min-h-[280px] space-y-1">
                {parsedLines.map((line, index) => {
                    const placeholder = index === 0 ? 'Write the idea here. Add context, questions, examples, next steps...' : '';

                    return (
                        <div key={`${index}-${line.type}`} className="flex min-h-8 items-start gap-3">
                            <div className="flex h-8 w-7 shrink-0 items-center justify-center pt-0.5 text-sm font-semibold text-slate-400">
                                {line.type === 'check' && (
                                    <input
                                        type="checkbox"
                            ...
                            <textarea
                                ...
                                className={`min-w-0 flex-1 resize-none overflow-hidden bg-transparent py-0.5 text-base leading-7 text-slate-700 outline-none placeholder:text-slate-400 ${
            ...
            <div className="mt-3 flex items-center gap-1 border-t border-slate-200 pt-3">
                <button
                    type="button"
...
const IdeasBoard = () => {
    ...
    return (
        <div className="mx-auto grid w-full max-w-6xl gap-6 px-1 py-2 lg:grid-cols-[320px_minmax(0,1fr)]">
            <aside className="min-w-0">
                <div className="mb-5">
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-500">
                        A quiet place to collect thoughts and grow them into branches.
                    </p>
                </div>

                <form onSubmit={handleCreateIdea} className="mb-4 flex items-center gap-2 border-b border-slate-200 pb-4">
                    <input
                        value={newTitle}
                    ...
                    <button
                        ...
                        className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-900 text-white transition hover:bg-slate-800 disabled:bg-slate-200"
                        title="Add idea"
                    >
                        <Plus size={16} />
                    </button>
                ...
                <div className="relative mb-3">
                    ...
                    <input
                        ...
                        className="w-full border-b border-slate-200 bg-transparent py-2 pl-7 pr-2 text-sm text-slate-800 outline-none placeholder:text-slate-400 focus:border-slate-400"
                        placeholder="Search"
                    />
                    {query && (
                        <button
                            type="button"
                            onClick={() => setQuery('')}
                            className="absolute right-1 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md text-slate-400 hover:bg-slate-100 hover:text-slate-700"
                            title="Clear search"
                        >
                            <X size={14} />
                        </button>
                ...
                <div className="max-h-[calc(100vh-250px)] space-y-1 overflow-y-auto pr-1">
                    ...
                    {!loading && roots.length === 0 && (
                        <div className="px-2 py-8 text-sm text-slate-500">No ideas yet.</div>
                    )}
                    {roots.map((node) => (
                        <TreeItem
                            key={node.id}
            ...
            <main className="min-w-0 border-t border-slate-200 pt-5 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
                {activeNode ? (
                    <div className="mx-auto max-w-3xl">
                            ...
                            <div className="flex items-center gap-1.5">
                                <button
                                    ...
                                    className={`flex h-9 w-9 items-center justify-center rounded-lg transition ${
                                        activeNode.focused ? 'bg-amber-100 text-amber-700' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-700'
                                    }`}
                                ...
                                <button
                                    ...
                                    className={`flex h-9 w-9 items-center justify-center rounded-lg transition ${
                                        activeNode.completed ? 'bg-emerald-100 text-emerald-700' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-700'
                                    }`}
                        ...
                        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-200 pt-4">
                            {IDEA_COLOR_OPTIONS.map((color) => (
                                <button
                                    ...
                                    className={`h-6 w-6 rounded-full border-2 transition ${getColors(color).dot} ${
                                        activeNode.color === color ? 'border-slate-900 scale-110' : 'border-white'
                                    }`}
                                    title={color}
                                />
                            ))}
                        ...
                        <div className="mt-8 border-t border-slate-200 pt-5">
                            <div className="mb-3 flex items-center justify-between gap-3">
                                <h2 className="text-sm font-bold uppercase tracking-[0.18em] text-slate-400">Branches</h2>
                                <span className="text-xs text-slate-400">{activeChildren.length}</span>
                            </div>
                            ...
                            <form onSubmit={handleAddChild} className="mb-4 flex items-center gap-2">
                                <input
                                    ...
                                    className="min-w-0 flex-1 border-b border-slate-200 bg-transparent px-1 py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-slate-400"
                                    placeholder="Add a branch..."
                                />
                                <button
                                    type="submit"
                                    disabled={!newChildTitle.trim()}
                                    className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 disabled:text-slate-200"
                                    title="Add branch"
                                >
                                    <Plus size={16} />
                                </button>
