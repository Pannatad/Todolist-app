...
const ScheduleBriefing = ({ briefing }) => {
  ...
  return <div className="mt-4 rounded-xl border border-sage-200/70 bg-white/70 p-3.5 dark:border-white/10 dark:bg-white/5"><p className="text-sm font-semibold leading-6 text-gray-700 dark:text-bone-200">{ahead ? <>You have <span className="font-black text-gray-950 dark:text-white">{briefing.remainingBlocks}</span> of {briefing.totalBlocks} {briefing.totalBlocks === 1 ? 'block' : 'blocks'} still ahead · {minutesLabel(briefing.remainingMinutes)} of scheduled time left</> : <>All <span className="font-black text-gray-950 dark:text-white">{briefing.totalBlocks}</span> {briefing.totalBlocks === 1 ? 'block is' : 'blocks are'} behind you · {minutesLabel(briefing.totalMinutes)} scheduled today</>} across {briefing.categories.length} {briefing.categories.length === 1 ? 'area' : 'areas'}.</p><div className="mt-3 flex flex-wrap gap-1.5">{briefing.categories.map((category) => <span key={category.category} className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold" style={{ backgroundColor: category.color.bgColor, color: category.color.color }}><span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: category.color.color }} />{category.category}<span className="opacity-70">{category.count} · {minutesLabel(category.minutes)}</span></span>)}</div></div>;
};
